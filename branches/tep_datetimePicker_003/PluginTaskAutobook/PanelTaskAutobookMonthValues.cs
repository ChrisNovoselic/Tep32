...
namespace PluginTaskAutobook
{
    public class PanelTaskAutobookMonthValues : HPanelTepCommon
    {
        ...
        /// 
        /// </summary>
        protected enum INDEX_CONTEXT { ID_CON = 10 }
        /// <summary>
        ...
        public class DGVAutoBook : DataGridView
        {
                ...
                , HandlerDbTaskCalculate.INDEX_TABLE_VALUES typeValues)
            {
                ...
                foreach (DataGridViewRow row in Rows)
                {
                    foreach (HDataGridViewColumn col in Columns)
                    {
                        ...
                        {
                            DataRow[] dr_CorValues = formingValueRaw(tbOrigin[(int)HandlerDbTaskCalculate.INDEX_TABLE_VALUES.DEFAULT]
                            ...
                            switch (typeValues)
                            {
                                ...
                                case HandlerDbTaskCalculate.INDEX_TABLE_VALUES.ARCHIVE:
                                    dr_Values = formingValueArch(tbOrigin[(int)typeValues],
                                        row.Cells["DATE"].Value.ToString()
                                        , col.m_iIdComp);
                                    showArchValues(dr_Values, row, col, idAlg, vsRatioValue);
                                    break;
                                case HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION:
                                    dr_Values = formingValueRaw(tbOrigin[(int)typeValues],
                                        row.Cells["DATE"].Value.ToString()
                                        , m_currentOffSet
                                        , col.m_iIdComp);
                                    showRawValues(dr_Values, row, col, idAlg, vsRatioValue, typeValues);
                                    break;
            ...
            /// <summary>
            /// Отображение сырых значений
            /// </summary>
            /// <param name="dr_Values">массив с данными</param>
            /// <param name="row">строка в гриде</param>
                ...
                , HandlerDbTaskCalculate.INDEX_TABLE_VALUES typeValues)
            {
                double dblVal = -1F;
                bool bflg = false;
                //HandlerDbTaskCalculate.INDEX_TABLE_VALUES typeValues = HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION;
            ...
            /// <param name="idAlg">номер алгоритма</param>
            /// <param name="vsRatioValue">значение множителя</param>
            private void showArchValues(DataRow[] dr_Values
                , DataGridViewRow row
                , HDataGridViewColumn col
                , int idAlg
                , int vsRatioValue)
            {
                double dblVal = -1F;
                bool bflg = false;
                HandlerDbTaskCalculate.INDEX_TABLE_VALUES typeValues = HandlerDbTaskCalculate.INDEX_TABLE_VALUES.ARCHIVE;
                ...
                if (dr_Values != null)
                    if (dr_Values.Count() > 0)
                        ...
                        for (int p = 0; p < dr_Values.Count(); p++)
                            if (row.Cells["DATE"].Value.ToString() ==
                            Convert.ToDateTime(dr_Values[p]["WR_DATETIME"]).AddDays(-1).ToShortDateString())
                            {
                                dblVal = correctingValues(Math.Pow(10F, vsRatioValue)
                                    , dr_Values[p]["VALUE"]
                                    , col.Name
            ...
            /// <param name="DataGridView">дата</param>
            ...
            public void editCells(int row, int value, DataGridView view, string col)
            {
                ...
                for (int i = row; i < view.Rows.Count; i++)
                    if (Int32.TryParse(view.Rows[row].Cells[INDEX_GTP.TEC.ToString()].Value.ToString(), out value))
                        fillCells(view.Rows[i]);
                    else
                        break;
            }
            ...
            public void countDeviation(DataGridViewRow row)
            {
                ...
                if (row.Cells["StSwen"].Value == null || row.Cells["StSwen"].Value.ToString() == "")
                    row.Cells["DevOfPlan"].Value = "";
                else
                    if (Int32.TryParse(row.Cells["PlanSwen"].Value.ToString(), out _number))
                        row.Cells["DevOfPlan"].Value = double.Parse(row.Cells["StSwen"].Value.ToString()) - _number;//??
                    else
            ...
            private DataRow[] formingValueArch(DataTable dtOrigin, string date, int idPut)
            {
                DateTime dateOffSet;
                ...
                {
                    dateOffSet = m_enumResIDPUT.ElementAt(i).DATE_TIME.AddDays(-1);
            ...
            /// <param name="idPut">идЭлемента</param>
            /// <returns>набор строк</returns>
            private DataRow[] formingValueRaw(DataTable dtOrigin, string date, int offSet, int idPut)
            {
                DateTime dateOffSet;
                DataRow[] dr_idCorPut = null;
            ...
            /// <param name="row">строка</param>
            public DataTable FillTableCorValue(DataGridView dgvView
                , int offset
                , object value
                , int column
                , int row)
                ...
                for (int i = 0; i < dgvView.Rows.Count; i++)
                {
                    foreach (HDataGridViewColumn col in Columns)
                    {
                        if (col.Index > (int)INDEX_GTP.GTP36 & col.Index < (int)INDEX_GTP.CorGTP36)
                        {
                            ...
                            {
                                valueToRes = Convert.ToDouble(value) * Math.Pow(10F, 1 * vsRatioValue);
            ...
            public DataTable FillTableValueDay(DataGridView dgvView, HandlerDbTaskCalculate.INDEX_TABLE_VALUES typeValues)
            {
                ...
                if (typeValues == HandlerDbTaskCalculate.INDEX_TABLE_VALUES.ARCHIVE)
                    _quality = 1;
        ...
        protected class PanelManagementAutobook : HPanelCommon
        {
            ...
            private void InitializeComponents()
            {
                ...
                //dtPickerCalendar.Anchor = (AnchorStyles)(AnchorStyles.Left | AnchorStyles.Right);
        ...
        private void InitializeComponent()
        {
            ...
            m_dgvAB.AddColumn("Корректировка ПТО,\r\nБлоки 1-2", false, INDEX_GTP.CorGTP12.ToString());
            m_dgvAB.AddColumn("Корректировка ПТО,\r\nБлоки 3-6", false, INDEX_GTP.CorGTP36.ToString());
        ...
        void PanelTaskAutobookMonthValue_btnsend_Click(object sender, EventArgs e)
        {
            ...
            m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION] = m_dgvAB.FillTableValueDay(m_dgvAB, m_ViewValues);
            //
        ...
        void dgvAB_CellParsing(object sender, DataGridViewCellParsingEventArgs e)
        {
            ...
            double.TryParse(m_dgvAB.Rows[e.RowIndex].Cells[m_dgvAB.Columns[e.ColumnIndex].Name].Value.ToString(), out valueCor);
            ...
            switch (m_dgvAB.Columns[e.ColumnIndex].Name)
            {
                case "CorGTP12":
                    ...
                    m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.DEFAULT] =
                        m_dgvAB.FillTableCorValue(m_dgvAB, Session.m_curOffsetUTC, value, e.ColumnIndex, e.RowIndex);
                    //сбор значений
                    m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION] =
                        m_dgvAB.FillTableValueDay(m_dgvAB, m_ViewValues);
                    break;
                case "CorGTP36":
                    if (value == 0)
        ...
        {
            m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.DEFAULT] =
                m_arTableOrigin[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.DEFAULT].Clone();
            m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION]
                = m_arTableOrigin[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.SESSION].Clone();
            m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.ARCHIVE]
              = m_arTableOrigin[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.ARCHIVE].Clone();
        }
        ...
        private void loadArchValues()
        {
            ...
            {
                // установить значения в таблицах для расчета, создать новую сессию
                setValues(HandlerDb.GetDateTimeRangeValuesVarArchive(), out err, out errMsg);
        ...
        private void valuesFence()
        {
            ...
            m_arTableEdit[(int)TepCommon.HandlerDbTaskCalculate.INDEX_TABLE_VALUES.DEFAULT] =
                m_dgvAB.FillTableCorValue(m_dgvAB, Session.m_curOffsetUTC);
        }
        ...
        protected override void initialize(out int err, out string errMsg)
        {
            ...
            for (int j = (int)INDEX_GTP.CorGTP12; j < (int)INDEX_GTP.COUNT; j++)
            {
                if (drEdtCol.Count() > 0)
                {
                    if (int.Parse(drEdtCol[0]["VALUE"].ToString()) == (int)MODE_CORRECT.ENABLE)
                        m_dgvAB.AddBRead(true, namePut.GetValue(j).ToString());
                    else
            ...
            if (err == 0)
            {
                try
                {
                    ...
                    for (int j = 0; j < drProf.Count(); j++)
                        if (Convert.ToInt32(drProf[j]["ID_ITEM"]) == (int)INDEX_CONTEXT.ID_CON)
