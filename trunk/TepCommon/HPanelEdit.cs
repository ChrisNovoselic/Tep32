...
namespace TepCommon
{
    public partial class HPanelEdit : TableLayoutPanel
    {
        ...
        protected void initialize (object obj) {
            ...
            {
                //((DataGridView)m_dictControls[(int)INDEX_CONTROL.DGV_DICT_EDIT]).SelectionChanged += new EventHandler(HPanelEdit_SelectionChanged);
                //((DataGridView)m_dictControls[(int)INDEX_CONTROL.DGV_DICT_EDIT]).Rows[0].Selected = false;
                //((DataGridView)m_dictControls[(int)INDEX_CONTROL.DGV_DICT_EDIT]).Rows[0].Selected = true;
            }
        }
